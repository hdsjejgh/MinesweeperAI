    ...
    def add_knowledge(self, cell, count):
        ...
        for column in range(cell[0]-1, cell[0]+2):
            for row in range(cell[1]-1, cell[1]+2):
                pass
                c = (column,row)
                if c == cell or c in self.safes or c in self.mines:
                    count -= 1 if c in self.mines else 0
                    continue
                if 0<=row<self.height and 0<=column<self.width:
                    uctiles.add(c)
        newsent = Sentence(uctiles, count)
        ...
        for subset in self.knowledge:
            ...
            for superset in self.knowledge:
                ...
                if subset.cells.issubset(superset.cells):
                    resultcells = superset.cells - subset.cells
                    resultmines = superset.count - subset.count
                    newsentence = Sentence(resultcells, resultmines)
